...
void CesiumEngine::createTileset(const std::string& token, int64_t assetId) {
...
void CesiumEngine::destroyTileset() {
  currentOverlay_ = nullptr; // Release intrusive ref before tileset destruction.
  tileset_.reset();
}
...
void CesiumEngine::setImageryAssetId(int64_t assetId) {
  ...
  // assetId == 1 means terrain-only (hypsometric) — no overlay added.
  if (assetId == 1) return;

  // Add a new IonRasterOverlay for the given asset.
  // IntrusivePointer is Cesium Native's intrusive reference-counted pointer.
  currentOverlay_ = new CesiumRasterOverlays::IonRasterOverlay(
      "imagery",
      assetId,
      config_.ionAccessToken);
  tileset_->getOverlays().add(currentOverlay_);
}
...
FrameResult CesiumEngine::updateFrame(double w, double h) {
  ...
  // Dispatch newly-completed load-thread work on the main thread.
